...
export function calcularPensionSobrevivencia(
  ...
  const advertencias = [
    `Pensión de referencia: ${formatearPesos(pensionReferencia)}`,
    `Total porcentajes: ${(porcentajeTotal * 100).toFixed(0)}%`,
    ...pensionPorBeneficiario.map(b => 
      `${tipoLabels[b.tipo] || b.tipo}: ${(b.porcentaje * 100).toFixed(0)}% = ${formatearPesos(b.pensionMensual)}`
    )
  ];
